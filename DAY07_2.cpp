...
ll dfs(ll r,ll c,vector<string>& vec,vector<vector<ll>>& dp){
    if(r==vec.size()) return 1;

    if(dp[r][c]!=-1) return dp[r][c];

    ll ans=0;
    if(vec[r][c]=='.' || vec[r][c]=='S') ans+=dfs(r+1,c,vec,dp);
    else{
        if(c-1>=0) ans+=dfs(r+1,c-1,vec,dp);
        if(c+1<vec[0].size()) ans+=dfs(r+1,c+1,vec,dp);
    }
